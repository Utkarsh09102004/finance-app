import os
import requests
import json
from dotenv import load_dotenv
from typing import Dict, Any, List, Optional

# Handle both package import and direct execution
try:
    from ...auth.token_manager import get_valid_access_token
except ImportError:
    import sys
    import os
    # Get the path to the zoho_books_mcp_server directory
    current_file = os.path.abspath(__file__)
    reports_dir = os.path.dirname(current_file)
    api_dir = os.path.dirname(reports_dir)
    zoho_dir = os.path.dirname(api_dir)
    src_dir = os.path.dirname(zoho_dir)
    server_root = os.path.dirname(src_dir)
    
    # Add parent of zoho_books_mcp_server to path
    sys.path.insert(0, os.path.dirname(server_root))
    
    # Now import using the full path from FinSync directory
    from zoho_books_mcp_server.src.zoho.auth.token_manager import get_valid_access_token

def extract_sales_by_customer_data(sales_json_data):
    """
    Extracts relevant Sales by Customer information from the provided JSON structure.
    
    Args:
        sales_json_data (dict): A dictionary containing the sales by customer report JSON data.
        
    Returns:
        dict: A dictionary structured for potential AI analysis, containing:
              - report_metadata: Context about the report (dates, pagination info).
              - summary: Aggregate statistics across all customers.
              - customers: Detailed sales data for each customer.
    """
    if sales_json_data.get("code") != 0 or not sales_json_data.get("sales"):
        print("Error or no sales data found in JSON.")
        return None

    sales_data = sales_json_data["sales"]
    page_context = sales_json_data.get("page_context", {})

    # Calculate summary statistics
    total_sales = sum(customer.get("sales", 0.0) for customer in sales_data)
    total_sales_with_tax = sum(customer.get("sales_with_tax", 0.0) for customer in sales_data)
    total_transactions = sum(customer.get("count", 0) for customer in sales_data)
    
    extracted_data = {
        "report_metadata": {
            "report_type": page_context.get("report_type", "sales_by_customer"),
            "from_date": page_context.get("from_date"),
            "to_date": page_context.get("to_date"),
            "page": page_context.get("page", 1),
            "per_page": page_context.get("per_page", 200),
            "has_more_pages": page_context.get("has_more_page", False),
            "entity_types": page_context.get("entity_list", "").split(","),
            "currency": "INR"  # Extracted from first customer if available
        },
        "summary": {
            "total_customers": len(sales_data),
            "total_sales": total_sales,
            "total_sales_with_tax": total_sales_with_tax,
            "total_transactions": total_transactions,
            "average_sales_per_customer": total_sales / len(sales_data) if sales_data else 0,
            "average_transactions_per_customer": total_transactions / len(sales_data) if sales_data else 0
        },
        "customers": []
    }

    # Extract currency from first customer if available
    if sales_data and sales_data[0].get("currency_code"):
        extracted_data["report_metadata"]["currency"] = sales_data[0]["currency_code"]

    # Process individual customer data
    for customer in sales_data:
        customer_info = {
            "customer_name": customer.get("customer_name"),
            "customer_id": customer.get("customer_id"),
            "transaction_count": customer.get("count", 0),
            "sales": customer.get("sales", 0.0),
            "sales_with_tax": customer.get("sales_with_tax", 0.0),
            "currency_code": customer.get("currency_code", "INR"),
            "sales_percentage": (customer.get("sales", 0.0) / total_sales * 100) if total_sales > 0 else 0
        }
        
        extracted_data["customers"].append(customer_info)

    # Sort customers by sales in descending order for better analysis
    extracted_data["customers"].sort(key=lambda x: x["sales"], reverse=True)

    return extracted_data

def fetch_sales_by_customer(organization_id=None, from_date=None, to_date=None, access_token=None):
    """
    Fetches sales by customer data from Zoho Books API.
    
    Args:
        organization_id (str, optional): Zoho organization ID. If None, gets from env vars.
        from_date (str): Start date in YYYY-MM-DD format
        to_date (str): End date in YYYY-MM-DD format
        
    Returns:
        dict: Formatted sales by customer data
    """
    load_dotenv()
    
    # Get organization ID from environment variables if not provided
    if not organization_id:
        organization_id = os.getenv("ZOHO_ORGANIZATION_ID")
    
    if not organization_id:
        print("Error: ZOHO_ORGANIZATION_ID not found in environment variables")
        return None
    
    # Get valid access token if not provided
    if not access_token:
        access_token = get_valid_access_token()
        if not access_token:
            print("Error: Failed to obtain a valid access token")
            return None
    
    # Prepare request headers
    headers = {
        "Authorization": f"Zoho-oauthtoken {access_token}"
    }
    
    # Define API endpoint for sales by customer report
    url = "https://www.zohoapis.in/books/v3/reports/salesbycustomer"
    
    # Set parameters - only essential ones
    params = {
        "organization_id": organization_id,
        "sort_order": "A",
        "sort_column": "customer_name",
        "filter_by": "TransactionDate.CustomDate",
        "from_date": from_date,
        "to_date": to_date,
        "entity_list": "invoice,creditnote"  # Include invoices and credit notes
    }
    
    try:    
        # Make the API request
        response = requests.get(url, headers=headers, params=params)
        
        # Check response status
        if response.status_code == 200:
            sales_data = response.json()
            
            # Extract relevant fields using the extraction logic
            relevant_data = extract_sales_by_customer_data(sales_data)
            
            return relevant_data
        else:
            print(f"API request failed with status code: {response.status_code}")
            print(f"Response: {response.text}")
            return None
    
    except requests.exceptions.RequestException as e:
        print(f"Error during API request: {str(e)}")
        return None
    except Exception as e:
        print(f"An unexpected error occurred: {str(e)}")
        return None


if __name__ == "__main__":
    # Test with fiscal year 2024-2025
    start_date = "2024-04-01"
    end_date = "2025-03-31"
    
    result = fetch_sales_by_customer(
        from_date=start_date,
        to_date=end_date
    )
    
    if result:
        print("\nExtracted sales by customer data:")
        print(json.dumps(result, indent=2))
    else:
        print("Failed to retrieve sales by customer data")