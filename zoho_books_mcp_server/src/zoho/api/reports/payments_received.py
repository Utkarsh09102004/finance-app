...
def extract_payments_received_data(payments_json_data):
    """
    Extracts relevant Payments Received information from the provided JSON structure.
    
    Args:
        payments_json_data (dict): A dictionary containing the payments received report JSON data.
    ...
    Returns:
        dict: A dictionary structured for potential AI analysis, containing:
              ...
              - payments: Detailed data for each payment received.
    """
    ...
    # Group by payment mode and customer
    payment_mode_totals = {}
    customer_totals = {}
    ...
    for payment in payment_data:
        ...
        customer_totals[customer] = customer_totals.get(customer, 0.0) + payment.get("amount", 0.0)
    
    extracted_data = {
        "report_metadata": {
            "report_type": page_context.get("report_type", "payments_received"),
        ...
        "summary": {
            ...
            "top_customers": [
                {"customer": customer, "total": amt, "percentage": (amt / total_amount * 100) if total_amount > 0 else 0}
                for customer, amt in sorted(customer_totals.items(), key=lambda x: x[1], reverse=True)[:10]
            ]
        },
        "payments": []
    }
    ...
    for payment in payment_data:
        payment_info = {
            ...
            "deposit_to_account_name": payment.get("deposit_to_account_name"),
            "deposit_to_account_id": payment.get("deposit_to_account_id"),
            "invoice_numbers": payment.get("invoice_numbers", ""),
            "unused_amount": payment.get("unused_amount", 0.0),
            "bank_charges": payment.get("bank_charges", 0.0),
...
def fetch_payments_received(organization_id=None, from_date=None, to_date=None, access_token=None):
    """
    Fetches payments received data from Zoho Books API.
    ...
    Returns:
        dict: Formatted payments received data
    """
    load_dotenv()
    ...
    # Define API endpoint for payments received report
    url = "https://www.zohoapis.in/books/v3/reports/customerpayments"
    ...
    try:    
        ...
        if response.status_code == 200:
            ...
            relevant_data = extract_payments_received_data(payments_data)
...
if __name__ == "__main__":
    ...
    result = fetch_payments_received(
        from_date=start_date,
        to_date=end_date
    )
    
    if result:
        print("\nExtracted payments received data:")
        print(json.dumps(result, indent=2))
    else:
        print("Failed to retrieve payments received data")